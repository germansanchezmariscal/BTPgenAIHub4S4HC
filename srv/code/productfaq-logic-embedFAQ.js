/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "german_1_STUDENTH01Srv.ProductFAQ")
...
try {
    ...
    productFAQ = await SELECT.one('German_1_STUDENTH01.ProductFAQ').where({ ID: productFAQID }).forUpdate();
    if (!productFAQ) {
...
try {
    ...
    await UPDATE('German_1_STUDENTH01.ProductFAQ').set({ embedding: embedding }).where({ ID });
